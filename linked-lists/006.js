...
function remove (element) {
    var to_remove = [], is_success = false, old_head = head, old_tail = tail;
    ...
    while (head !== tail && head !== undefined) {
...
function insertAfter (element, data) {
    var new_ele, is_success = false, old_head = head, old_tail = tail;

    if (data['checkList']) {
        new_ele = data;
    } else {
        new_ele = new List(data);
    }
    if (head === tail) {
        if (head === element) {
            head.next = new_ele;
